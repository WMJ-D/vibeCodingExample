...
router.get('/operation', authorize('log:operation:list'), asyncHandler(async(req,res)=>{
  ...
  const[rows]=await getDb().query(`SELECT id,trace_id,module,operation_type,description,operator_username,request_method,request_url,ip_address,status,error_message,duration_ms,operated_at FROM sys_operation_log WHERE ${where} ORDER BY operated_at DESC LIMIT ? OFFSET ?`,[...values,pageSize,offset])
...
router.get('/operation/export',authorize('log:operation:export'),asyncHandler(async(req,res)=>{const{where,values}=operationWhere(req.query);const[rows]=await getDb().query(`SELECT module,operation_type,description,operator_username,ip_address,status,error_message,duration_ms,operated_at FROM sys_operation_log WHERE ${where} ORDER BY operated_at DESC LIMIT 10000`,values);sendCsv(res,'operation-logs.csv',[{key:'module',label:'操作模块'},{key:'type',label:'操作类型'},{key:'description',label:'操作描述'},{key:'operator',label:'操作人'},{key:'ip',label:'操作IP'},{key:'statusText',label:'状态'},{key:'error_message',label:'错误信息'},{key:'duration_ms',label:'耗时(ms)'},{key:'operated_at',label:'操作时间'}],rows.map(row=>({...row,type:displayType[row.operation_type]||row.operation_type,operator:row.operator_username,ip:row.ip_address,statusText:row.status===1?'成功':'失败'})))}))
